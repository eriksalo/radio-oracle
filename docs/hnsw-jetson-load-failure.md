...
**Resolved 2026-05-18** via option 3 (switch backend). The two-tier RAG migration in `rag-migration-runbook.md` rebuilt every collection with `nomic-ai/nomic-embed-text-v1.5` + FAISS IVF-PQ. Wikipedia's FAISS index is 840 MB and loads into RAM cleanly on the Jetson; gutenberg follows the same pattern. The chromadb collections remain on the workstation as the read-only source of truth for chunk text, but nothing live reads them at runtime any more.

Diagnostic kept here for future debugging of any chromadb-HNSW deployment.
